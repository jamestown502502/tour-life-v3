...
export class HubScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(W / 2, 60, State.data.band.name, {
    ...
    if (stop) {
      ...
      this.add.text(W / 2, 780, `Next stop: ${city.name}`, {
      ...
      this.add.text(W / 2, 780, 'The last show is behind you.', {
        fontFamily: 'Georgia, serif', fontSize: '22px', color: PALETTE_HEX.cream,
      }).setOrigin(0.5);
    ...
    if (State.data.meta.unlockedDecor.length > 0) {
      this.add.text(W / 2, 920, `Décor: ${State.data.meta.unlockedDecor.map((d) => d.replace(/_/g, ' ')).join(', ')}`, {
        fontFamily: 'Georgia, serif', fontSize: '14px', color: PALETTE_HEX.sky, wordWrap: { width: W - 100 }, align: 'center',
      }).setOrigin(0.5);
    }
  ...
  private renderStats(): void {
    ...
    keys.forEach((key, i) => {
      ...
      const barW = 280;
      this.add.text(60, y, STAT_LABELS[key], { fontFamily: 'Georgia, serif', fontSize: '16px', color: PALETTE_HEX.cream });
      ...
      this.add.text(220 + barW + 12, y, key === 'funds' ? `$${value}` : `${Math.round(value)}`, {
        fontFamily: 'Georgia, serif', fontSize: '14px', color: PALETTE_HEX.cream,
      });
    });
  }
